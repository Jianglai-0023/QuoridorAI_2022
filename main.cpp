...
public:
    ...
    std::pair<int, double> Selection(const State &s, int N) {//<win,UCTofson,son>
        ...
        if (!tree[s].ismet) {
            tree[s].ismet = true;
            tree[s].son = next_step(s);
            tree[s].unexpand_num = tree[s].son.size();
        }
        if (tree[s].unexpand_num) {//未完全展开
            ...
            for (auto i = n.son.begin(); i != n.son.end(); ++i) {//expand一次
                if (!tree[*i].ismet) {//未被访问过的节点，进行simulation
                    ...
                    int n = ++tree[*i].N;
                    int q = tree[*i].Q += win;
                    ans.second = tree[*i].UCT = cal_UCT(double(n), double(q), double(N + 1));
